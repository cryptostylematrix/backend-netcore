...
public sealed class ResetStructureActivityRequestConsumerTests
{
    [Fact]
    public async Task Resets_activity_for_every_place_in_the_requested_structure()
    {
        var places = new[]
        {
            ...
            CreatePlace("inactive-with-date", isActive: false, activatedAt: 10)
        };
        ...
        try
        {
            var client = provider.GetRequiredService<IClientFactory>()
                .CreateRequestClient<ResetStructureActivaityRequest>();
            var response = await client.GetResponse<IntegrationRequestResponse>(
                new ResetStructureActivaityRequest(
                    "marketing",
                    2,
                    Guid.NewGuid(),
                    DateTime.UtcNow));
    ...
    private static Place CreatePlace(string profile, bool isActive, long? activatedAt) =>
        Place.Create(
            parentId: 1,
